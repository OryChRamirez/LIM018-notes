...
export class DropdownMenuComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.service.$closeModalsOfDropdown.subscribe((valor) => {
      //CIERRA TODOS LOS MODALES CUANDO SE DA CLICK EN ALGUN MODAL DEL HEADER
    ...
    this.service.$changeColorWhite.subscribe((valor) =>
      this.renderer.setStyle(this.notesIcon.nativeElement, 'color', valor)
    );
  ...
  activeDropdownMenu() {
    // MUESTRA U OCULTA LAS OPCIONES DEL MENU LATERAL AL DARLE CLICK
    if (this.dropdownMenu === true) {
      this.dropdownMenu = false;
      this.renderer.setStyle(
        this.btnDropdownMenu.nativeElement,
        'align-self',
        'center'
      );
      this.renderer.setStyle(
        this.seccContDropdownMenu.nativeElement,
        'width',
        'max-content'
      );
      this.renderer.setStyle(
        this.labelListStyle.nativeElement,
        'border-top',
        'none'
      );
    } else {
  ...
  showModalChangeNickname() {
    //MUESTRA EL MODAL PARA CAMBIAR EL NICK DEL USUARIO
  ...
  targetElementNickname(event: any) {
    //LIMPIA EL NOMBRE DE USUARIO AL DARLE CLICK AL INPUT
    this.changeNicknameUser.nativeElement.value = '';
  }

  async changeNickname() {
    //FUNCIÓN PARA CAMBIAR EL NICK DEL USUARIO EN FIRESTORE
    let nickname = this.changeNicknameUser.nativeElement.value;
    this.modalChangeNameUser = false;
    const response = await this.service.changeNicknameOrName(
      this.currUser,
      nickname
    );
  }
  ...
  closeModalChangeNickname() {
    // CIERRA EL MODAL DE CAMBIO DE NOMBRE EN CASO DE CANCELAR LA ACCIÓN
    this.modalChangeNameUser = false;
  }

  editLabelById(idLabel: string) {
    //ABRE EL MODAL DE EDITAR ETIQUETA
    this.service.$closeModalsOfHeader.emit(false);
    this.service.$showModelStickyNoteFromDropdown.emit(false);
    this.modalChangeNameUser = false;
  ...
  deleteLabelById(idLabel: string) {
    //ABRE EL MODAL DE BORRAR ETIQUETA
    this.service.$closeModalsOfHeader.emit(false);
    this.service.$showModelStickyNoteFromDropdown.emit(false);
    this.arrLabelsByUser.forEach((label) => {
      if (idLabel === label.id) {
  ...
  cancelDeleteLabel() {
    //FUNCIÓN PARA CANCELAR EL ELIMINAR ETIQUETA
    this.modalDeleteLabel = false;
  }

  changeLabel() {
    //FUNCIÓN PARA EDITAR EL NOMBRE Y COLOR DE LA ETIQUETA
    const color = this.inputColor.nativeElement.value;
    const label = this.labelName.nativeElement.value;
    const idUser = this.currUser;
    const labelExist = this.arrLabelsByUser.find(
      (labelData) => labelData.nameLabel === label
    );
    if (labelExist !== undefined || label === 'Nuevo Nombre...') {
      this.msgError.nativeElement.innerHTML =
        'Elige otro nombre para la etiqueta';
    } else {
      this.msgError.nativeElement.innerHTML = '';
  ...
  closeModalEditLabel() {
    //CIERRA EL MODAL DE EDITAR ETIQUETA
    this.modalEditLabel = false;
  }

  targetElementnewLabel(event: any) {
    //LIMPIA EL NOMBRE DE LA ETIQUETA AL DARLE CLICK AL INPUT
